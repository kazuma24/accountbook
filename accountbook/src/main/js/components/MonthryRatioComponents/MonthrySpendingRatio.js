...
class MonthrySpendingRatio extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
                        ...
                        <table>
                            ...
                            <tbody>
                                    {state.amountData.length != 0 && state.amountData.map(data => {
                                        return (
                                            <tr className="report-amount" key={data.spendingCategoryName}>
    ...
    componentDidMount(){
        ...
        axios.post("/acquisitionofeachamountdata",params)
        .then(response => {
...
export default withRouter(MonthrySpendingRatio)