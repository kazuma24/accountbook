...
public interface SpendingAndIncomeMapper {
	...
	@Select("WITH "
			...
			+ "SELECT "
			+ "COALESCE((SELECT * FROM s),0) AS totalMonthlySpending, "
	...
	//指定の年の合計収支を取得する
		@Select("WITH "
				...
				+ "SELECT "
				+ "COALESCE((SELECT * FROM s),0) AS totalYearSpending, "
	...
	//指定年月の各日にちの支出データを取得
		...
		@Select("SELECT "
				...
				+ "account_id, "
				+ "concat(income_year, to_char(income_month,'00'), to_char(income_day,'00')) as startdate "
				+ "FROM income "
				+ "WHERE account_id = #{accountId} "
				+ "AND income_year = #{year} "
				+ "AND income_month = #{month} "
				+ "GROUP BY "
				+ "income_day, account_id, income_year, income_month, income_day;")
