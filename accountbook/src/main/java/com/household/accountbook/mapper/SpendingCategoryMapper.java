...
public interface SpendingCategoryMapper {
	...
	// 新規追加
	@Select("INSERT INTO spending_category " + "VALUES ("
	...
	@Select("UPDATE spending_category " + "SET "
			+ "spending_category_name = #{afterName}, spending_category_color = #{afterColor} " + "WHERE "
			+ "account_id = #{accountId} AND spending_category_name = #{beforeName}; " + "UPDATE spending " + "SET "
			+ "spending_category_name = #{afterName} " + "WHERE "
	...
	@Select("DELETE FROM spending_category " + "WHERE "
