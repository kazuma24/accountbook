...
public interface IncomeCategoryMapper {
	...
	//デフォルト収入カテゴリテーブルの値を収入カテゴリテーブルにインサートして、インサートした値を取得（初期ユーザー）
		@Select("INSERT INTO income_category "
				+ "(account_id, income_category_name, income_category_color) "
				+ "SELECT #{id}, default_income_category_name, default_income_category_color "
				+ "FROM "
				+ "default_income_category "
				+ "returning income_category_name, income_category_color") 
		...
		@Select("UPDATE income_category "
				+ "SET "
				+ "income_category_name = #{afterName}, income_category_color = #{afterColor} "
				+ "WHERE "
				+ "account_id = #{accountId} AND income_category_name = #{beforeName}; "
				+ "UPDATE income "
				+ "SET "
				+ "income_category_name = #{afterName} "
				+ "WHERE "
				+ "account_id = #{accountId} AND income_category_name = #{beforeName};"
		...
		@Select("DELETE FROM income_category "
				+ "WHERE "
